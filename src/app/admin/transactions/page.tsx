...
export default async function TransactionsPage() {
  ...
  const ACCENT_COLORS = ['#2563EB', '#60A5FA', '#34D399', '#FBBF24', '#60A5FA'];
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div>
        ...
        <p className="text-sm mt-0.5" style={{ color: '#334155' }}>Comprehensive transaction insights and trends</p>
      </div>

      {/* KPI Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={DollarSign}    label="Total Revenue"     value={`₹${totalRevenue.toFixed(2)}`}     accentColor="#34D399" iconBg="rgba(16,185,129,0.18)"  sub={`${coinPayments?.length || 0} coin + ${transactions?.length || 0} online + ${rfidPayments?.length || 0} rfid`} />
        ...
        <StatCard icon={TrendingDown}  label="Failed"             value={String(failedTransactions.length)}  accentColor="#2563EB" iconBg="rgba(37,99,235,0.18)"  sub="Failed payments" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl p-5" style={CARD}>
          <h3 className="font-semibold text-slate-900 mb-1">Daily Revenue</h3>
          <p className="text-xs mb-5" style={{ color: '#64748b' }}>Last 7 days</p>
                ...
                <div key={i} className="flex-1 flex flex-col items-center gap-2">
                  <div className="w-full flex flex-col items-center justify-end h-40 relative group">
                    <div
                      ...
                      style={{ background: '#ffffff', border: '1px solid #e2e8f0', color: '#0f172a', boxShadow: '0 8px 24px rgba(15,23,42,0.12)' }}
                    >
                      <div className="font-semibold">₹{day.revenue.toFixed(2)}</div>
                      <div style={{ color: '#334155' }}>{day.count} txn</div>
                    </div>
                    <div
          ...
          <div className="mt-4 pt-3 flex items-center justify-between text-xs" style={{ borderTop: '1px solid #f1f5f9', color: '#334155' }}>
        ...
        {/* Top Products */}
        <div className="rounded-2xl p-5" style={CARD}>
          <h3 className="font-semibold text-slate-900 mb-1">Top Products by Revenue</h3>
          <p className="text-xs mb-5" style={{ color: '#64748b' }}>Across both payment methods</p>
          <div className="space-y-4">
            {topProducts.length > 0 ? topProducts.map(([product, revenue], i) => {
              ...
              return (
                <div key={i}>
                  <div className="flex items-center justify-between mb-1.5 text-sm">
                    <span className="text-slate-900 font-medium truncate flex-1">{product}</span>
                    <span className="font-bold ml-4" style={{ color }}>₹{revenue.toFixed(2)}</span>
                  </div>
                  <div className="w-full h-2 rounded-full overflow-hidden" style={{ background: '#f8fafc' }}>
                    <div
              ...
              <p className="text-center py-8 text-sm" style={{ color: '#64748b' }}>No product data available</p>
            )}
          </div>
        </div>
      </div>
      ...
      {/* Top Machines */}
      <div className="rounded-2xl p-5" style={CARD}>
        <h3 className="font-semibold text-slate-900 mb-4">Top Machines by Revenue</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {topMachines.length > 0 ? topMachines.map(([machine, revenue], i) => (
            <div
              key={i}
              className="p-4 rounded-xl"
              style={{ background: '#ffffff', border: `1px solid ${ACCENT_COLORS[i % ACCENT_COLORS.length]}30` }}
            >
              <p className="text-xs mb-1" style={{ color: ACCENT_COLORS[i % ACCENT_COLORS.length] }}>#{i + 1}</p>
              <p className="font-medium text-slate-900 text-sm truncate mb-2">{machine}</p>
              <p className="text-lg font-bold" style={{ color: ACCENT_COLORS[i % ACCENT_COLORS.length] }}>₹{revenue.toFixed(2)}</p>
            </div>
          )) : (
            <p className="col-span-5 text-center py-4 text-sm" style={{ color: '#64748b' }}>No machine data available</p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl overflow-hidden" style={CARD}>
        <div className="px-5 py-4" style={{ borderBottom: '1px solid #f1f5f9' }}>
          ...
          <p className="text-xs mt-0.5" style={{ color: '#64748b' }}>Last 50 coin, online &amp; RFID payments</p>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {coinPayments && coinPayments.map((txn: any) => (
                ...
                >
                  <td className="py-3 px-4 whitespace-nowrap" style={{ color: '#334155' }}>
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-1.5 text-sm">
                      <User className="w-3.5 h-3.5 shrink-0" style={{ color: '#64748b' }} />
                      <span style={{ color: '#334155' }}>Coin Payment</span>
                    </div>
                  </td>
                  <td className="py-3 px-4 text-slate-900">{txn.products?.name || 'Unknown'}</td>
                  <td className="py-3 px-4">
                    <p className="text-slate-900">{txn.vending_machines?.name || 'Unknown'}</p>
                    <p className="text-xs" style={{ color: '#64748b' }}>{txn.vending_machines?.location}</p>
                  </td>
                  <td className="py-3 px-4 text-right">
                    <span className="font-semibold text-slate-900 flex items-center justify-end gap-0.5">
                  ...
                  <td className="py-3 px-4 text-center">
                    <span className="inline-block px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: 'rgba(16,185,129,0.15)', color: '#047857' }}>Dispensed</span>
                  </td>
                </tr>
              ))}
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1.5">
                        <User className="w-3.5 h-3.5 shrink-0" style={{ color: '#64748b' }} />
                        <span className="text-slate-900 text-sm">{txn.profiles?.email || 'Guest'}</span>
                      </div>
                    </td>
                    <td className="py-3 px-4 font-medium text-slate-900">{productNames}</td>
                    <td className="py-3 px-4" style={{ color: '#334155' }}>{txn.vending_machines?.name || 'N/A'}</td>
                    <td className="py-3 px-4 text-right">
                      <span className="font-semibold text-slate-900 flex items-center justify-end gap-0.5">
              ...
              {rfidPayments && rfidPayments.map((txn: any) => (
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-1.5 text-sm">
                      <User className="w-3.5 h-3.5 shrink-0" style={{ color: '#64748b' }} />
                      <span style={{ color: '#334155' }}>{txn.rfid_cards?.holder_name || txn.card_uid}</span>
                    </div>
                  </td>
                  <td className="py-3 px-4 text-slate-900">{txn.products?.name || 'Unknown'}</td>
                  <td className="py-3 px-4">
              ...
              {!transactions?.length && !coinPayments?.length && !rfidPayments?.length && (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-sm" style={{ color: '#64748b' }}>No transactions found</td>
                </tr>
              )}
            </tbody>
          </table>
