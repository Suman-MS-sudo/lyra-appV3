...
export default async function AnalyticsPage() {
  ...
  const ACCENT_COLORS = ['#2563EB', '#60A5FA', '#34D399', '#FBBF24', '#60A5FA', '#2563EB'];
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: '#334155' }}>Comprehensive business insights and trends</p>
        </div>
        <div
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={DollarSign} label="Total Revenue" value={`₹${totalRevenue.toFixed(2)}`} accentColor="#34D399" iconBg="rgba(16,185,129,0.18)"
        ...
        <StatCard icon={Activity} label="Total Machines" value={String(totalMachines || 0)} accentColor="#60A5FA" iconBg="rgba(96,165,250,0.18)"
      ...
      <div className="rounded-2xl overflow-hidden" style={CARD}>
        <div className="flex items-center gap-2 px-5 py-4" style={{ borderBottom: '1px solid #f1f5f9' }}>
          ...
          <div>
            <h2 className="font-semibold text-slate-900">Machines by Organization</h2>
            <p className="text-xs" style={{ color: '#64748b' }}>Machine distribution across customer organizations</p>
          </div>
        </div>
        <div className="p-5">
          {organizationMachineList.length > 0 ? (
              ...
              {organizationMachineList.map((org, i) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-slate-900 text-sm truncate">{org.name}</p>
                    <p className="text-xs mt-0.5" style={{ color: '#64748b' }}>Customer organization</p>
                  </div>
                  <div
            ...
            <p className="text-center py-8 text-sm" style={{ color: '#64748b' }}>No machine data available</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {topProductsList.length > 0 ? topProductsList.map((product, i) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs mt-0.5" style={{ color: '#64748b' }}>{product.count} sales</p>
                </div>
        ...
        <div className="rounded-2xl overflow-hidden" style={CARD}>
          <div className="px-5 py-4" style={{ borderBottom: '1px solid #f1f5f9' }}>
            ...
            <p className="text-xs mt-0.5" style={{ color: '#64748b' }}>Transaction count by machine</p>
          </div>
              ...
              return (
                <div key={i}>
                  <div className="flex items-center justify-between text-sm mb-1.5">
                    <span className="text-slate-900 font-medium truncate max-w-40">{machine.name}</span>
                    <div className="flex items-center gap-3 shrink-0">
                      <span style={{ color: '#334155' }}>{machine.count} txn</span>
      ...
      <div className="rounded-2xl overflow-hidden" style={CARD}>
        <div className="px-5 py-4" style={{ borderBottom: '1px solid #f1f5f9' }}>
          ...
          <p className="text-xs mt-0.5" style={{ color: '#64748b' }}>Revenue breakdown by customer organization</p>
        </div>
              ...
              {organizationList.length > 0 ? organizationList.map((org, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-center">
                      <span className="inline-block px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: '#f8fafc', color: '#0f172a' }}>{totalPurchases}</span>
                    </td>
                    ...
                    <td className="py-3 px-4 text-right" style={{ color: '#93C5FD' }}>₹{org.rfidRevenue.toFixed(2)}</td>
                    <td className="py-3 px-4 text-right font-bold" style={{ color: '#34D399' }}>₹{orgTotal.toFixed(2)}</td>
                  </tr>
                );
              }) : (
                <tr>
                  <td colSpan={9} className="py-12 text-center text-sm" style={{ color: '#64748b' }}>No organization data available</td>
                </tr>
              )}
            </tbody>
          </table>
            ...
            <tbody>
              {recentCombined.length > 0 ? recentCombined.map((tx) => (
                <tr
                  ...
                  style={{ borderBottom: '1px solid #f1f5f9' }}
                >
                  <td className="py-3 px-4" style={{ color: '#334155' }}>{new Date(tx.created_at).toLocaleDateString('en-IN')}</td>
                  <td className="py-3 px-4 font-medium text-slate-900">{tx.machineName}</td>
                  <td className="py-3 px-4" style={{ color: '#334155' }}>{tx.productName}</td>
                  <td className="py-3 px-4 text-center">
                    <span
                  ...
                  <td className="py-3 px-4 text-right font-semibold text-slate-900">₹{tx.total_amount.toFixed(2)}</td>
                  <td className="py-3 px-4 text-center">
                    <span
                      className="inline-block px-2 py-0.5 rounded-full text-xs font-medium"
                      style={
                ...
                <tr>
                  <td colSpan={6} className="py-12 text-center text-sm" style={{ color: '#64748b' }}>No transactions found</td>
                </tr>
              )}
            </tbody>
          </table>
